...
namespace MatrixLibrary
{
    public class LinearSystem
    {
        ...
        public Vector GaussBackword()
        {
            Vector vectorResult = new Vector(xVector.Size);
            int indexCountVector = xVector.Size - 1; //позиция для заполнения
            int sizeColl = 1; //сколько необходимо пройти столбцов
            for (int j = aMatrix.Row - 1; j >= 0; j--)
            {
                for (int i = aMatrix.Coll - 1; i >= aMatrix.Coll - sizeColl; i--)
                {
                    ...
                    {
                        bVector.Values[indexCountVector] -=(aMatrix.Values[j, i] * vectorResult.Values[i]) ;
                    }
                }
                sizeColl++;
            }
