    ...
    public class DistributedSolution
    {
        ...
        private void WorkClient(int sizeMessage)
        {
            ...
            while (checkClient < Client.NumberClient - 1)
            {
                ...
                {
                    double[] getParameters = Converter.GetDoubles(Client.Receive(buffer));
        ...
        {
            Client.Send(Converter.GetBytes(new int[1] { Matrix.Row * Matrix.Coll }));
            Client.Send(Converter.GetBytes(Matrix.ConvertMatrixToArray()));
        }
